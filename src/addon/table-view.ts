...
const handleMarkdownEditing = (cell, cm, tableData, rowIndex, colIndex) => {
  ...
  const findTokenElementAtCursor = () => {
    ...
    while (node && node !== cell) {
      if (node.nodeType === Node.ELEMENT_NODE && (node as HTMLElement).hasAttribute("data-token")) {
        return node;
      }
...
CodeMirror.defineOption("hmdTableView", defaultOption, function (cm: cm_t, newVal: OptionValueType) {
  ...
  const enabled = !!newVal //replace this hardcoded with the !!newVal
...
/** ADDON GETTER (Singleton Pattern): a editor can have only one TableView instance */
export const getAddon = Addon.Getter("TableView", TableView, defaultOption)
